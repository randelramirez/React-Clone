...
export function useMessages(channelId: string) {
  ...
  const handleNewMessage = useCallback((data: { message: Message }) => {
    if (data.message.channel === channelId) {
      queryClient.setQueryData(['messages', channelId], (oldMessages: Message[] | undefined) => {
        ...
        const messageExists = messages.some(msg => msg._id === data.message._id);
    ...
    if (data.message.channel === channelId) {
      queryClient.setQueryData(['messages', channelId], (oldMessages: Message[] | undefined) => {
        ...
        return messages.map(msg => {
          if (msg._id === data.message._id) return data.message;
          return msg;
        });
      });
    }
  ...
  const handleMessageDeleted = useCallback((data: { messageId: string; channelId: string }) => {
    if (data.channelId === channelId) {
      queryClient.setQueryData(['messages', channelId], (oldMessages: Message[] | undefined) => {
        const messages = oldMessages || [];
        return messages.filter(msg => msg._id !== data.messageId);
      });
    }
  }, [channelId, queryClient]);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      socket.off('message:edited', handleMessageDeleted);
      socket.off('message:deleted', handleMessageDeleted);
      socket.emit('channel:leave', { channelId });
    };
  }, [socket, channelId, handleNewMessage, handleMessageEdited, handleMessageDeleted]);
